...
//            url: Wtf.req.base+'UserManager.jsp',
        ...
        this.cm = new Wtf.grid.ColumnModel(
            ...
            this.sm2,
            {
                ...
                header :WtfGlobal.getLocaleText("hrms.common.ReportingTo"),
                dataIndex: 'manager',
                autoSize : true,
                sortable: true,
                pdfwidth:120,
       ...
       menubtns.push(
               this.addtimesheetView = new Wtf.Action({
                   text:WtfGlobal.getLocaleText("hrms.timesheet.timesheet"),
                   iconCls:getButtonIconCls(Wtf.btype.viewbutton),
                   id:this.id+'addtimesheetViewrejected',
                   tooltip:WtfGlobal.getLocaleText("hrms.timesheet.timesheet.tooltip"),
                   disabled:true,
                   scope:this,
                   handler:this.addtimesheet
               }));    
...
//        }));
        ...
        if(isStandAlone){
        	empbtns.push('-',this.getUserManagementBtns());
        }
        ...
        this.sm2.on("selectionchange",function(){
        	...
        	if(this.sm2.getCount()==0){
        		//menubtns Start
        		this.deleteUser.setDisabled(true);
        		this.viewEditProfile.setDisabled(true);
        		this.addtimesheetView.setDisabled(true);
        		...
        		this.addtimesheetView.setDisabled(false);
    ...
    addtimesheet:function(){ 
        ...
        var roleid=rec[0].get('roleid');
        var main=Wtf.getCmp("empmanagement");
        var edsignupTab1=Wtf.getCmp("timesheetemp");
        if(edsignupTab1==null)
        {
            edsignupTab1=new Wtf.timesheetemp({
                title:WtfGlobal.getLocaleText("hrms.timesheet.timesheet"),
                closable:true,
                id:'addtimesheet'+empid,
                layout:'fit',
                viewtimesheet:false,
                editperm:perm,
                blockemployeestoedit:false,
...
//        this.objsearchComponent.searchFlag = 1;
    ...
    getUserManagementBtns: function(){
    	this.createUser = new Wtf.Action({
            text:WtfGlobal.getLocaleText("hrms.common.CreateUser"),
    	...
    	this.deleteUser = new Wtf.Action({
            text:WtfGlobal.getLocaleText("hrms.stanalone.delete.user"),
            ...
            handler:function(){
    			...
    			var rec = this.sm2.getSelections();
    			for(var i=0; i<this.sm2.getCount(); i++){
    				if(loginid==rec[i].data.userid){
    					msgBoxShow([WtfGlobal.getLocaleText("hrms.common.warning"), WtfGlobal.getLocaleText("hrms.stanalone.user.delete.error.msg")],0);
    					return;
    				}else{
    					userids.push(rec[i].data.userid);
    				}
    			}
	    		Wtf.MessageBox.confirm(WtfGlobal.getLocaleText("hrms.stanalone.delete.user"), WtfGlobal.getLocaleText("hrms.stanalone.delete.user.msg"), function(btn){
		            if(btn!="yes") return;
		            Wtf.Ajax.requestEx({
		            ...
		            function(response) {
		                var responseObj = eval('('+response+')');
		            	if(responseObj.data.creator && responseObj.data.othersdeleted){
		            		msgBoxShow([WtfGlobal.getLocaleText("hrms.common.warning"), WtfGlobal.getLocaleText("hrms.stanalone.creator.cannot.delete.msg.other.deleted")],0);
			            	this.empGDS.load({params:{grouper:'usergrid',start:0,limit:this.allempGrid.pag.pageSize}});
		            	}else if(responseObj.data.creator){
		            		msgBoxShow([WtfGlobal.getLocaleText("hrms.common.warning"), WtfGlobal.getLocaleText("hrms.stanalone.creator.cannot.delete.msg")],0);
		            	}else{
		            		msgBoxShow([WtfGlobal.getLocaleText("hrms.common.success"), WtfGlobal.getLocaleText("hrms.stanalone.delete.user.success.msg")],1);
			            	this.empGDS.load({params:{grouper:'usergrid',start:0,limit:this.allempGrid.pag.pageSize}});	
		            	}
		            },
		            function(){
    	...
    	var menubtns=[this.createUser, this.edituser, this.deleteUser];
    	
    	var userManagement= new Wtf.Toolbar.Button({
            text:WtfGlobal.getLocaleText("hrms.common.UserManagement"),
            iconCls:'user-management',
            tooltip: {
                title:WtfGlobal.getLocaleText("hrms.common.UserManagement"),
                text: WtfGlobal.getLocaleText("hrms.standalone.create.manage.user")
            },
            menu:new Wtf.menu.Menu({
                items:menubtns
            })
