    ...
    initComponent:function (){
        ...
        this.masterSm.on("selectionchange",function(){
            ...
            if(this.masterSm.getSelected()){
                ...
                if(this.projectGrid!=null)this.newP.remove(this.projectGrid);
                ...
                else if(this.masterSm.getSelected().get("id")==26){//Projects
                	
                    this.newP.add(this.getprojectGrid());
...
//            disabled:true,
    ...
    AddMasterData:function (action){
        ...
        this.addeditmaster.on('show',function(){
            this.addeditmaster.name.focus(true,300);
        },this);
...
//        },this);
    ...
    AddProject:function (action){
        if(action!='Add'){
        var configid = this.projectGrid.getSelectionModel().getSelected().get("id");
        var parentid = this.projectGrid.getSelectionModel().getSelected().get("parentid");
        }
        this.addproject = new Wtf.AddProject({
            layout:"fit",
            iconCls:getButtonIconCls(Wtf.btype.winicon),
            title:WtfGlobal.getLocaleText({key:"hrms.common.project.params",params:[action]}),
            modal:true,
            configid:configid,
            parentid:parentid,
            width:450,
            height:300,
            rec:this.projectSm.getSelected(),
            action:action,
            store:this.projectStore
        })

    this.addproject.on('show',function(){
        this.addproject.name.focus(true,300);
    },this);
...
//                url: Wtf.req.base + "hrms.jsp",
    ...
    DeleteProject:function (){
        Wtf.MessageBox.confirm(WtfGlobal.getLocaleText("hrms.common.confirm"), deleteMsgBox('field'),function(btn){
            ...
            //alert("configId"+this.projectSm.getSelected().get("id"));
            calMsgBoxShow(201,4,true);
            Wtf.Ajax.requestEx({
//                url: Wtf.req.base + "hrms.jsp",
                url:"Common/Master/deletemasterdata.common",
                params:{
                    flag:48,
                    ids:this.projectSm.getSelected().get("id")
                }
            },
            this,
            function (response){
...
//                url: Wtf.req.base + "hrms.jsp",
    ...
    getprojectGrid:function (){
        this.projectRec = new Wtf.data.Record.create([
        {
            name:"id"
        },
        ...
        this.projectReader = new Wtf.data.KwlJsonReader1({
            root:"data",
            totalProperty:"count"
        },this.projectRec);
        ...
        this.projectStore.on("load",function(){
            if(msgFlag==1)
                WtfGlobal.closeProgressbar();
        },this);

        this.projectColumn = new Wtf.grid.ColumnModel([
            new Wtf.grid.RowNumberer(),
            {
                header:WtfGlobal.getLocaleText("hrms.common.sub.job.name"),
                sortable:true,
                autoWidth : true,
                dataIndex:"name"
            },
            {
                header:WtfGlobal.getLocaleText("hrms.common.sub.job.code"),
                sortable:true,
                autoWidth : true,
                dataIndex:"code"
            }
        ...
        this.projectAdd = new Wtf.Toolbar.Button({
            text:WtfGlobal.getLocaleText("hrms.MasterData.add.project"),
            tooltip:WtfGlobal.getLocaleText("hrms.MasterData.list.project.tooltip"),
            iconCls:getButtonIconCls(Wtf.btype.addbutton),
            handler:function (){
                this.AddProject("Add");
            },
//            disabled:true,
        ...
        this.projectEdit = new Wtf.Toolbar.Button({
            text:WtfGlobal.getLocaleText("hrms.MasterData.edit.project"),
            tooltip:WtfGlobal.getLocaleText("hrms.MasterData.project.edit"),
            iconCls:getButtonIconCls(Wtf.btype.editbutton),
            handler:function (){
                this.AddProject("Edit");
            },
            scope:this,
            disabled:true
        });

        this.projectDelete = new Wtf.Toolbar.Button({
            text:WtfGlobal.getLocaleText("hrms.MasterData.delete.project"),
            tooltip:WtfGlobal.getLocaleText("hrms.MasterData.remove.project"),
            iconCls:getButtonIconCls(Wtf.btype.deletebutton),
            handler:function (){
                this.DeleteProject();
            },
        ...
        var projectbtn=new Array();
        if(!WtfGlobal.EnableDisable(Wtf.UPerm.masterconf, Wtf.Perm.masterconf.manage)){
        	projectbtn.push(this.projectAdd);
        	projectbtn.push('-');
        	projectbtn.push(this.projectEdit);
        	projectbtn.push('-');
        	projectbtn.push(this.projectDelete);
        }
        this.projectGrid = new Wtf.grid.GridPanel({
            sm:this.projectSm = new Wtf.grid.RowSelectionModel(),
            store:this.projectStore,
            //region:"center",
            cm:this.projectColumn,
            viewConfig:{
                forceFit:true
            },
            bbar:projectbtn,
            emptyText: WtfGlobal.emptyGridRenderer(WtfGlobal.getLocaleText("hrms.MasterData.select.project.subfields"))
        });
        this.projectSm.on("selectionchange",function (){
            if(this.projectSm.getSelected()){
                this.projectEdit.enable();
                this.projectDelete.enable();
            } else {
                this.projectEdit.disable();
                this.projectDelete.disable();
            }
        },this);
