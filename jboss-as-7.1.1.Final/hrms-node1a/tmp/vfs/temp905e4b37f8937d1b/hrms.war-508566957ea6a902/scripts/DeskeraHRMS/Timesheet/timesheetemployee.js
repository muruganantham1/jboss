...
var dateColumnHeaderStart=3;
Wtf.timesheetemp2=function(config){
    Wtf.timesheetemp2.superclass.constructor.call(this,config);
}
Wtf.extend(Wtf.timesheetemp2,Wtf.Panel,{
	autoScroll :false,
    initComponent:function(config){
        Wtf.timesheetemp2.superclass.initComponent.call(this,config);
    },
    onRender: function(config) {
        Wtf.timesheetemp2.superclass.onRender.call(this, config);
        ...
        { name:'jobtype'}, { name:'subjobtype'},{name:'col1'},{name:'col2'},{name:'col3'},{name:'col4'},{name:'col5'},
        { name:"col6"},{name:"col7"},{name:'colid1'},{name:'colid2'},{name:'colid3'},
        { name:'colid4'},{name:'colid5'},{name:"colid6"},{name:"colid7"},{name:"total"},{name:"isSubmitted"},{name:"isAdmin"}
        ]);
...
//            url:Wtf.req.base + "hrms.jsp",
        ...
        if(!jobtypeflag){
           ...
           checkForJobTypeStoreLoad1();
           

            this.jobtext = new Wtf.form.ComboBox({
                store:Wtf.jobtypeStoreApp,
                displayField:'name',
                valueField:'id',
                scope:this,
                mode:'local',
        ...
        var subjobtypeflag = Wtf.cmpPref.timesheetjob;
        if(!subjobtypeflag){
		checkForSubJobTypeStoreLoad1();
		
			
			this.subjobtext = new Wtf.form.ComboBox({
                store:Wtf.subjobtypeStoreApp,
                displayField:'name',
                valueField:'id',
                scope:this,
                mode:'local',
        	   ...
        	   this.subjobtext=new Wtf.ux.TextField({
               name:'subjobtypetextbox',
               width:200,
               allowBlank:false,
               validator:WtfGlobal.noBlankCheck,
               maxLength:100
		...
		{   
            header: WtfGlobal.getLocaleText("hrms.timesheet.project"),//"SubJob",
            width:300,
            fixed:true,
            sortable: true,
            dataIndex: 'subjobtype',
            summaryRenderer:WtfGlobal.totalSummaryRenderer,
			renderer:(!subjobtypeflag)?Wtf.comboBoxRenderer(this.subjobtext):"",
            editor: (this.isadmin)?this.subjobtext:"",
            align:'center'
        },
        {
            header: WtfGlobal.getLocaleText("hrms.timesheet.activity"),//"Job",
            width:300,
            fixed:true,
            sortable: true,
            dataIndex: 'jobtype',
            ...
            editor: (this.isadmin)?this.jobtext:"",
            align:'center'
        },
        {
        ...
        this.jobgrid = new Wtf.grid.EditorGridPanel({
            ...
            tbar:(this.isadmin)?[(WtfGlobal.getLocaleText("hrms.timesheet.fromdate")+":"),//'From Date:',
            this.fromdate,'-',(WtfGlobal.getLocaleText("hrms.timesheet.todate")+":"),this.todate,'-',this.submitjobs]:
            	[(WtfGlobal.getLocaleText("hrms.timesheet.fromdate")+":"),//'To Date:',this.todate]:['From Date:',
            this.fromdate,'-',(WtfGlobal.getLocaleText("hrms.timesheet.todate")+":"),this.todate],
            cm: this.cm1,
            sm: this.sm
        });
        if(!jobtypeflag) {
	...
	var hours;
        if(e.field=='jobtype' || e.field=='subjobtype'){
...
//                this.jobstore.data.items[e.row].set('col7',hours);
    ...
    validEditFunction:function(e){
        ...
        var col1Total = 0;
        var col2Total=0;
        if(e.column==3||e.column==4||e.column==5||e.column==6||e.column==7||e.column==8||e.column==9)
        {
            if(aa[0]>24||aa[1]>=61)
            {
                calMsgBoxShow(144,0);
       ...
       /* if(e.column == 2){
        ...
        for(var tmp1=0;tmp1<this.jobstore.getCount();tmp1++){
            if(tmp1!=e.row)
            {
				...
				var columnCounter = dateColumnHeaderStart;
                if(e.column==columnCounter++){
                    aa=this.jobstore.getAt(tmp1).get("col1").split(":");
    ...
    changegridheader: function(){
        ...
        var mychange=(this.day==WtfGlobal.getLocaleText("hrms.Monday").substring(0,3)?0:(this.day==WtfGlobal.getLocaleText("hrms.Tuesday").substring(0,3)?1:(this.day==WtfGlobal.getLocaleText("hrms.Wednesday").substring(0,3)?2:(this.day==WtfGlobal.getLocaleText("hrms.Thursday").substring(0,3)?3:(this.day==WtfGlobal.getLocaleText("hrms.Friday").substring(0,3)?4:(this.day==WtfGlobal.getLocaleText("hrms.Saturday").substring(0,3)?5:(this.day==WtfGlobal.getLocaleText("hrms.Sunday").substring(0,3)?6:7)))))));
        var myDate=new Date();
        ...
        this.cm1.setColumnHeader(dateColumnHeaderStart,myDate.format('D d M Y'))
        for(i=dateColumnHeaderStart;i<dateColumnHeaderStart+6;i++)
        {
            myDate.setDate(myDate.getDate()+1);
            this.dateArray.push(myDate.format('Y-m-d'))            
    ...
    gridcalculation:function(isSubmittedFlag){
        ...
        for(var i=0;i<this.jobstore.getCount();i++){
			...
			jsondata += "'subjobtype':'" + WtfGlobal.onlySinglequoateRenderer(this.jobstore.getAt(i).get("subjobtype")) + "',";
...
Wtf.extend(Wtf.StartTimer,Wtf.Window,{
    ...
    GetNorthPanel:function (){
        ...
        this.northPanel = new Wtf.Panel({
            ...
            height:70,
            border:false,
            bodyStyle:"background-color:white;border-bottom:1px solid #FFF;",
