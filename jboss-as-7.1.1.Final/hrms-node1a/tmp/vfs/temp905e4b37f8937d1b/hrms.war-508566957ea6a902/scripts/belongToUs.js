...
*/
var LOGIN_PREFIX = 'AuthHandler/verifyLogin.common';
var EMAIL_ACTION='Common/forgetPassword.common';
var SIGNUP_PREFIX = 'signup.jsp?';
var DUMMY_PREFIX = 'selection.jsp?flag=2';
...
function validateLogin(u, p){
   ...
   if(p.length==0)
    	p=p;
    		...
    		p=p;
//    	    p =hex_sha1(p);
//    alert($('#domain')[0].value);
    var p = 't=a&x=' + encodeURI(hex_hmac_sha1(d, p) + '&u=' + encodeURI(u) + '&p=' + encodeURI(p) + '&dc=' + encodeURI(d)+'&dm='+encodeURI($('#domain')[0].value));
...
function dummyvalidateLogin(){
    var rad_val="";
    var pass="";
    var oldpass="";
    if(document.dummyform.dummy.length!=undefined) {
        for (var i=0; i < document.dummyform.dummy.length; i++) {
            if (document.dummyform.dummy[i].checked) {
                rad_val = document.dummyform.dummy[i].value;
            }
        }
    } else {
        rad_val = document.dummyform.dummy.value;
    }
    ...
    if(document.getElementById("newpassword").value.length>=4) {
        if(document.getElementById("newpassword2").value==document.getElementById("newpassword").value){
            document.getElementById("passwd_td").innerHTML="";
            document.getElementById("oldpasswd_td").innerHTML="";
            document.getElementById("submt").disabled=true;
            oldpass=document.getElementById("newpassword").value ;
            pass=document.getElementById("newpassword2").value ;
            var p='radio='+encodeURI(rad_val)+'&pass='+encodeURI(hex_sha1(pass))+'&oldpass='+encodeURI(hex_sha1(oldpass))+'';
            http.open('POST', DUMMY_PREFIX, true);
            http.setRequestHeader("Content-length", 500);
            http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            http.onreadystatechange = dummyhandleHttpValidateLogin;
            http.send(p);
        }else {
            document.getElementById("passwd_td").innerHTML="<font color=red>Passwords do not match.</font>";
            document.getElementById("oldpasswd_td").innerHTML="";
        }
    }else {
        document.getElementById("oldpasswd_td").innerHTML="<font color=red>Length should be at least 4 characters long.</font>";
        document.getElementById("passwd_td").innerHTML="";
    }
}

function dummyhandleHttpValidateLogin(){
    if (http.readyState == NORMAL_STATE) {
        if(http.responseText!=""){
            var results = eval("(" + trimStr(http.responseText) + ")");
            //document.getElementById("formContainerDiv").innerHTML = http.responseText;
          redirect(results.location);
        }
    }
}
    ...
    if (http.readyState == NORMAL_STATE) {
        if (http.responseText && http.responseText.length > 0) {
            ...
            if (results.success == true) {
                ...
                SetCookie("superuser", results.superadmin);
...
function signUpUser(u, p, eml, cn,fn, ad){
...
//    var country = $('#country')[0];
    ...
    //var aD = (!address.value || trimStr(address.value).length == 0 || trimStr(address.value).length > 32 || address.value.replace(/[^\w\s_\-\'\"]+/g, '') != address.value) ? $("#address")[0].className = "validation errorimg" : $("#address")[0].className = "validation valid";
