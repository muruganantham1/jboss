#!/bin/sh
# Generated with YourKit Java Profiler 12.0.6
...
#JAVA_OPTS="-agentpath:/opt/jprofiler/bin/linux-x64/libjprofilerti.so=port=8580,nowait $JAVA_OPTS"

#export JAVA_OPTS="$JAVA_OPTS -Dcom.sun.management.jmxremote.port=8780 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false"
...
if [ "$PRESERVE_JAVA_OPTS" != "true" ]; then
    ...
    if [ "x$JVM_D32_OPTION" != "x" ]; then
        ...
        "$JAVA"  -agentpath:/opt/yjp-12.0.6/bin/linux-x86-64/libyjpagent.so=disablestacktelemetry,disableexceptiontelemetry,builtinprobes=none,delay=10000,sessionname=JBoss -d32 $JAVA_OPTS -version > /dev/null 2>&1 && PREPEND_JAVA_OPTS="-d32" && JVM_OPTVERSION="-d32"
    fi
    ...
    if [ $CLIENT_VM = false ]; then
        ...
        if [ "x$NO_COMPRESSED_OOPS" = "x" ]; then
            "$JAVA"  -agentpath:/opt/yjp-12.0.6/bin/linux-x86-64/libyjpagent.so=disablestacktelemetry,disableexceptiontelemetry,builtinprobes=none,delay=10000,sessionname=JBoss $JVM_OPTVERSION -server -XX:+UseCompressedOops -version >/dev/null 2>&1 && PREPEND_JAVA_OPTS="$PREPEND_JAVA_OPTS -XX:+UseCompressedOops"
        fi

        NO_TIERED_COMPILATION=`echo $JAVA_OPTS | $GREP "\-XX:\-TieredCompilation"`
        if [ "x$NO_TIERED_COMPILATION" = "x" ]; then
            "$JAVA"  -agentpath:/opt/yjp-12.0.6/bin/linux-x86-64/libyjpagent.so=disablestacktelemetry,disableexceptiontelemetry,builtinprobes=none,delay=10000,sessionname=JBoss $JVM_OPTVERSION -server -XX:+TieredCompilation -version >/dev/null 2>&1 && PREPEND_JAVA_OPTS="$PREPEND_JAVA_OPTS -XX:+TieredCompilation"
        fi
    fi
...
while true; do
   if [ "x$LAUNCH_JBOSS_IN_BACKGROUND" = "x" ]; then
      ...
      eval \"$JAVA\"  -agentpath:/opt/yjp-12.0.6/bin/linux-x86-64/libyjpagent.so=disablestacktelemetry,disableexceptiontelemetry,builtinprobes=none,delay=10000,sessionname=JBoss -D\"[Standalone]\" $JAVA_OPTS \
